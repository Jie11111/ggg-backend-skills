---
name: ggg-prd-intake
description: 需求入口和需求理解阶段。GGG 根据用户表达和影响面自动路由 quick/full 并告知，只有模式冲突或依据不足时才阻塞询问；以 PRD、用户消息、会议结论、原型等需求材料为主，必要时只读核实代码事实。高影响阻塞问题一轮一问，同模块低风险独立问题可合并 2-3 个，确认后形成 quick 边界或正式 baseline。仅当用户明确指定 `$ggg-prd-intake`，或要求需求受理、梳理 PRD 基线时使用。
---
...
GGG 作为接需求的 Java 程序员，先判断 `quick/full` 并告知，再澄清真正影响业务边界的问题。材料已明确的内容直接记录，不为走流程重复确认。代码现状不等于用户需求，AI 推荐和常见做法也不能替代用户结论。
...
## 第一步：自动路由 quick / full 并告知

路由清楚时直接继续，不等待用户重复确认：

```text
已按 quick / full 推进：<一句话依据>。如需切换模式可以直接说明。
```

路由规则：

- 用户明确指定 quick/full 且与已知风险不冲突时，直接采用。
...
- 只有模式信号冲突，或材料不足以安全判断时，才问一次模式问题并给出推荐；回答前不初始化目录。
...
validate 通过后进入 `$ggg-requirement-alignment`。
